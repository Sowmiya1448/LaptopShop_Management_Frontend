        ...
        let pay_data = pay.length > 0 && pay.find(pays => pays.customer_ref === data._id)
         ...
         return (
            <tr key={index}>
                ...
                <td>{totalpurchase}</td>
                <td><button className='btn btn-outline-success' onClick={() => navigate(`/pay/${data._id}`)}>pay</button></td>
                <td>{pay_data && pay_data.paid}</td>
                <td>{pay_data && pay_data.Balance}</td>
                <td><button className="btn btn-warning" onClick={() => navigate(`/payupdate/${data._id}`)}>Update</button></td>
